...
def get_command_router(memory: ConversationMemory, db_service: DatabaseService = None, gemini_service: GeminiService = None) -> Router:
    ...
    async def cmd_start(message: types.Message):
        ...
        try:
            ...
            logging.error(f"Error in cmd_start HTML send: {e}, attempting clean text fallback")
            clean_text = re.sub(r'<[^>]+>', '', welcome_text)
            await message.answer(
                clean_text,
                reply_markup=get_welcome_inline_keyboard()
            )
    ...
    @router.message(F.text == "💖 បរិច្ចាគ 2,000 ៛")
    async def cmd_donate(message: types.Message):
        """
        Handle /donate command to prompt user to select or enter donation amount.
        """
        if message.from_user:
            await _register_user(message.from_user, message.bot)

        from keyboards.inline import get_donation_amount_keyboard
